...
export async function POST(req: Request) {
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0a0a0a;color:#fff;padding:32px;border-radius:12px;border:1px solid #C9A84C33">
      <h2 style="color:#C9A84C;margin-bottom:24px">Nouvelle demande de rendez-vous</h2>
      <table style="width:100%;border-collapse:collapse">
        <tr><td style="padding:8px 0;color:#999;width:140px">Nom</td><td style="padding:8px 0;color:#fff">${name}</td></tr>
      ...
      <p style="margin-top:24px;font-size:12px;color:#555">SY MA — syhenna.vercel.app</p>
    </div>
  `

  const { error } = await resend.emails.send({
    from: 'SY MA <onboarding@resend.dev>',
